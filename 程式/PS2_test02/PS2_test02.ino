...
void loop(){
 ...
 if(type == 1) { // 這是標準的DualShock控制器
    ...
    if(ps2x.ButtonPressed(PSB_SELECT))       // 查看「選擇」鍵是否被按住
      Serial.println("Select is being held");
    
         
    if(ps2x.ButtonPressed(PSB_PAD_UP)) {         // 若「上」按鍵被按著
    ...
    if(ps2x.Button(PSB_RED)==true) {            // 若「按下」圈圈按鍵
         Serial.println("Circle just pressed");
         if(testangle>0){
         testangle-- ;
         test.write(testangle);
         }
 }  
    if(ps2x.Button(PSB_PINK)==true) {          // 若「放開」方塊按鍵
    ...
    if(ps2x.ButtonPressed(PSB_BLUE)) {           // 若「按下」或「放開」叉叉按鍵
